...
with cloud-init, generates the QEMU launch script, and launches it. Cloud-init Phase 2 (guest install) runs ~20-37 min on Ubuntu and ~42-129 min on Gentoo on an idle host (measured range; Gentoo varies with binhost warmth); when it's done the VM reboots into GDM and you log in
...
| GNOME Extensions GUI | shipped in `gnome-shell` (26.04+; standalone `gnome-extensions-app` removed from the archive) | `gnome-extra/gnome-shell-extensions` |
| Bluetooth daemon | `bluez` | `net-wireless/bluez` |
| GVFS backends (MTP / gphoto2) | `gvfs-backends` (umbrella) | `gnome-extra/gvfs` (umbrella) |
| Mesa demos (glxgears, glxinfo) | `mesa-utils` | `x11-apps/mesa-progs` |
...
| Firmware | UEFI | UEFI |
| Typical first-boot time (Phase 2, measured) | ~20-37 min | ~0.7-2.2 h (binhost + source builds) |
