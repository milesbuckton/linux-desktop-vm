...
A unified automation that installs **Ubuntu LTS or Gentoo** with
**GNOME on Wayland** inside a **QEMU** VM on **Apple Silicon (aarch64)**,
fully unattended. GPU-accelerated
graphics via **VirGL** (hardware-rendered OpenGL in the guest through the
`qemu-virgl` tap) and **Venus** (hardware-accelerated Vulkan over virtio). The script
...
with cloud-init, generates the QEMU launch script, and launches it. Cloud-init Phase 2 (guest install) runs ~20-48 min on Ubuntu and ~42-210 min on Gentoo on an idle host (measured range; Gentoo varies with binhost warmth and host load); when it's done the VM reboots into GDM and you log in
...
> on your behalf. If QEMU is not found on PATH,
> the script prints platform-specific install hints and exits.

Works on macOS hosts (Apple Silicon only — Intel Macs are not supported):
- **Apple Silicon** (aarch64 guests — the validated and only supported path): QEMU (HVF)

### Guest CPU architecture

Guests run **aarch64 on Apple Silicon** — the validated and only supported
path (the full distro fleet matrix has been built out on aarch64 guests).

### Virtual hardware

The guest always sees virtio-sound audio + xHCI USB 3, so PipeWire/USB code paths are identical. Disk controller,
NIC, GPU, and guest-agent stack are picked by the script's Jinja2
...
| **Audio controller** | virtio-sound-pci (`virtio_snd`) |
| **USB controller** | xHCI (`xhci_pci`) |
| **GPU host device** | `virtio-gpu-gl-pci` when the QEMU build has virgl; else `virtio-gpu-pci` (e.g. stock Homebrew QEMU) |
| **GPU kernel driver** | `virtio_gpu.ko` |
| **Mesa gallium driver** | `virgl` (`virgl_dri.so`) with a virgl QEMU; `llvmpipe` (software) otherwise |
| **3D acceleration** | Hardware GL (VirGL) + hardware Vulkan (**Venus**) with a QEMU build that has virglrenderer; software (llvmpipe / Lavapipe) otherwise |
| **Vulkan** | Hardware (Venus, Vulkan-over-virtio) when the QEMU build ships a Venus-capable virglrenderer with a working host Vulkan ICD (MoltenVK); otherwise Lavapipe (software Vulkan) |
| **Display session** | Wayland |
| **Mouse integration** | USB tablet (absolute coords; no capture/release) |
| **Window resolution** | 3456×2234 — QEMU `virtio-gpu-gl-pci,xres=3456,yres=2234` plus kernel `video=3456x2234@60` on every distro  |
...
| **VM definition file** | `launch-vm.sh` (QEMU argv) |

The guest is configured identically across distros from a desktop
...
One GNOME Shell extension is installed and enabled system-wide (dconf default
`05-extensions`): **dash-to-dock** (`dash-to-dock@micxgx.gmail.com`). The guest
fetches the zip from a **host-served local HTTP server** (QEMU's `10.0.2.2`
gateway) during cloud-init — **not** directly from GitHub, whose connection is
flaky at first boot — so installs are deterministic. A shell-version gate skips
the install if the release does not declare the guest's GNOME major.

| Extension | Source | Installs on |
|---|---|---|
| dash-to-dock (`dash-to-dock@micxgx.gmail.com`) | GitHub | GNOME 48–50 (every distro) |

The GNOME Extensions GUI is installed on every distro, so the user can install
any other extension themselves from the Extensions app. On Ubuntu 26.04 the
Extensions app ships inside the `gnome-shell` package itself
(`org.gnome.Shell.Extensions`) — the old standalone `gnome-extensions-app`
...
### Native packages (both distros)

Same app on both distros; the exact package name differs by distro (the
[Package naming differences](#package-naming-differences) legend below is
the canonical per-name reference).

| Slot | What it is |
|---|---|
| **Firefox** | Preinstalled web browser on both distros (Ubuntu: apt `firefox` snap shim; Gentoo: `www-client/firefox`). **Epiphany is the default browser** (see GNOME desktop defaults); Firefox stays installed as an alternative. |
| **GNOME Tweaks** | Advanced desktop customization (fonts, titlebar, shell themes) — User Themes is pre-enabled and the Shell theme is pinned to the distro default (see GNOME desktop defaults below) |
...
| **GNOME Loupe** | Default GNOME image viewer (replaces Eye of GNOME) — native on both distros (Gentoo unmasks `media-gfx/loupe ~arm64`) |
| **Snapshot (camera)** | Default GNOME camera (Cheese's successor) — on Gentoo installed via Flathub (`org.gnome.Snapshot`, no ebuild yet); native on Ubuntu |
...
| **Mesa demos (glxgears, glxinfo, eglinfo)** | GL diagnostics — required by `scripts/check-virgl.sh` |
| **Vulkan tools (vulkaninfo)** | Vulkan diagnostics — required by `scripts/check-venus.sh` |
| **fastfetch** | Terminal system-info tool (neofetch successor) |
| **Google Chrome** | Best-effort vendor-repo install ⁴ — `google-chrome-stable`, native package name identical on both. Installs on **aarch64** (Google ships arm64/aarch64 Linux builds). Also **simulate-validated** (see ⁴). |
| **Flatpak + Flathub** | `flatpak` framework preinstalled on both + the Flathub remote registered (GNOME Software's Flatpak plugin installs from it). |
...
| **PowerShell** | Best-effort universal tarball install (`pwsh`) — pulled from the GitHub release for the guest arch and symlinked to `/usr/local/bin/pwsh`. Not package-manager tracked (no auto-update); chosen over the Microsoft apt repo because it lags the newest distro bases (Ubuntu 26.04). Emits `POWERSHELL-OK` / `POWERSHELL-MISSING` in the cloud-init log (best-effort ⁴). |
...
LibreOffice is preinstalled on both distros (pulled in by the desktop meta-packages).

### GNOME desktop defaults (dconf, both distros)
...
| `org.gnome.shell enabled-extensions` | `dash-to-dock` + **User Themes** | Dock plus the User Themes extension (`user-theme@gnome-shell-extensions.gcampax.github.com`, ships in each distro's `gnome-shell-extensions` package) enabled by default, so GNOME Tweaks shows the Shell dropdown out of the box. |
| `org.gnome.shell.extensions.user-theme name` | `Yaru-dark` (Ubuntu) / `Adwaita` (Gentoo) | Tweaks → Appearance → Shell pinned to the distro's default shell theme (Ubuntu's dark Yaru variant, matching `prefer-dark`; GNOME's Adwaita on Gentoo). |
| Default browser (`x-scheme-handler/http(s)`, `text/html`) | **Epiphany** (`org.gnome.Epiphany.desktop`) | Set system-wide via `/etc/xdg/mimeapps.list` + per-user `xdg-settings`, so links open in Epiphany. Firefox and Chrome remain installed as alternatives. Marker: `BROWSER-DEFAULT-OK`. |
...
the package name is almost always identical (Ubuntu uses apt; Gentoo uses
portage `category/name` atoms). Exceptions are footnoted.

| Capability | apt (Ubuntu) | portage (Gentoo) |
|---|---|---|
| GNOME desktop meta | `ubuntu-desktop-minimal` | `gnome-base/gnome` (via `@gnome-set` meta) |
| Display manager | `gdm3` | `gnome-extra/gdm` |
| GNOME Tweaks | `gnome-tweaks` | `gnome-extra/gnome-tweaks` |
| GNOME Software | `gnome-software` | `gnome-extra/gnome-software` (flatpak via USE) |
| Console (terminal) | `gnome-console` | `gui-apps/gnome-console` |
| Text editor | `gnome-text-editor` | `gui-apps/gnome-text-editor` |
| Camera (Snapshot) | `gnome-snapshot` | (Flathub `org.gnome.Snapshot` — not in tree) |
| Sound recorder | `gnome-sound-recorder` | `gui-apps/gnome-sound-recorder` |
| PDF viewer (Papers) ² | `papers` | `app-text/papers` |
| Image viewer | `loupe` | `media-gfx/loupe` |
| Video player (Totem) | `totem` | `media-video/totem` |
| Mail (Geary) | `geary` | `mail-client/geary` |
| Web browser (Firefox) | `firefox` (snap shim) | `www-client/firefox` |
| Web browser (Epiphany) | `epiphany-browser` | `www-client/epiphany` |
| GNOME Bluetooth UI | `gnome-bluetooth` | `gnome-extra/gnome-bluetooth` |
| GNOME Extensions GUI | shipped in `gnome-shell` (26.04+; standalone `gnome-extensions-app` removed from the archive); extension packs via `gnome-shell-extensions` | `gnome-extra/gnome-shell-extensions` |
| Bluetooth daemon | `bluez` | `net-wireless/bluez` |
| GVFS backends (MTP / gphoto2) | `gvfs-backends` (umbrella) | `gnome-extra/gvfs` (umbrella) |
| Mesa demos (glxgears, glxinfo, eglinfo) | `mesa-utils` | `x11-apps/mesa-progs` + eglinfo built from `mesa/demos` into `/usr/local/bin` (Gentoo packages no eglinfo) |
| Vulkan tools (vulkaninfo, vkcube) | `vulkan-tools` | `dev-util/vulkan-tools` (vulkaninfo only — `cube` USE is masked on arm64, so `vkcube` is built from upstream source during install) |
| Vulkan loader + drivers | `mesa-vulkan-drivers` + `vulkan-loader` | `media-libs/vulkan-loader` + `media-libs/mesa` |
| PipeWire | `pipewire` + `pipewire-pulse` | `media-video/pipewire` + `media-sound/pipewire-pulse` |
| ALSA plugin | `pipewire-alsa` | `media-sound/pipewire-alsa` |
| JACK plugin | `pipewire-jack` | `media-libs/pipewire-jack` |
| WirePlumber | `wireplumber` | `media-video/wireplumber` |
| rtkit | `rtkit` | `sys-auth/rtkit` |
| Python pip ³ | `python3-pip` (+ `python3-venv`) | `dev-python/pip` |
| Fira Code font | `fonts-firacode` | `media-fonts/fira-code` |
| fastfetch | `fastfetch` | `app-misc/fastfetch` |
| lsb-release | `lsb-release` | `sys-apps/lsb-release` |
| net-tools (ifconfig) | `net-tools` | `sys-apps/net-tools` |
| usbutils (lsusb) | `usbutils` | `sys-apps/usbutils` |
| rclone | `rclone` | `net-misc/rclone` |
| Google Chrome | `google-chrome-stable` | `www-client/google-chrome` |
| Flatpak runtime | `flatpak` | `sys-apps/flatpak` |
| Flatseal (Flathub) | `com.github.tchx84.Flatseal` | `com.github.tchx84.Flatseal` |
| Gear Lever (Flathub) | `it.mijorus.gearlever` | `it.mijorus.gearlever` |
| GNOME backgrounds | `gnome-backgrounds` | `x11-themes/gnome-backgrounds` |
| Icon theme (Adwaita) | `adwaita-icon-theme` | `x11-themes/adwaita-icon-theme` |
| GTK themes (standard) | `gnome-themes-extra` | `x11-themes/gnome-themes-standard` |
| Mesa userspace GL stack | (pulled in by desktop meta) | (pulled in by `gnome-base/gnome`) |

² Both distros now install the `papers` package; Ubuntu retired the GNOME-47-era `evince` name in this release.

³ Ubuntu installs `python3-venv` explicitly alongside `python3-pip` for consistency. On Gentoo, venv is part of the Python base package.

⁴ **Best-effort vendor/third-party installs** — Google Chrome and the Flathub apps are best-effort: a vendor-repo or network blip must never sink the build. The guest's cloud-init log records `CHROME-OK` / `CHROME-MISSING` and `FLATPAK-APPS-OK` / `FLATPAK-APPS-MISSING` markers. Chrome is ALSO in the simulate dry-run lists — its vendor repo is registered in `bootcmd`, which runs in simulate mode too — so a Google repo or arch regression surfaces at the simulate gate instead of as a marker in a multi-hour build.
...
| `linux_vm/` | Core package: orchestrator, QEMU launcher, download, templates, monitor |
...
| `templates/ubuntu.j2` | Ubuntu LTS (standalone; inlined the former apt-family content) |
...
| `templates/_app_platforms_common.j2` | Shared vendor-app platform installs (Google Chrome, Flathub Flatseal/Gear Lever, PowerShell) |
...
detects it and refuses to start
...
You need the system emulator (`qemu-system-aarch64` on Apple Silicon),
`qemu-img`, and the OVMF firmware blobs for UEFI boot.

| Host | Install command |
|------|-----------------|
| macOS (Apple Silicon) | `brew install qemu`. HVF acceleration is built into macOS — no extra step. |
| macOS (hardware GL) | Optional: `brew tap milesbuckton/qemu-virgl && brew install milesbuckton/qemu-virgl/qemu-virgl`. QEMU **11.0.3** with virglrenderer + ANGLE/Metal. Prebuilt **Apple Silicon bottles** (arm64_tahoe/sequoia) are published as GitHub Release assets and poured by Homebrew — no source build; from-source builds (full Xcode) only if no bottle matches. Verified working on this project's Apple Silicon MacBook Pro host. The launcher auto-detects the GL device (`virtio-gpu-gl-pci`), uses `-display cocoa,gl=es`, and enables **Venus** (guest Vulkan over virtio) via the host's MoltenVK ICD; without it the guest renders GL with Mesa `llvmpipe` (software) and Vulkan with Lavapipe. Note the tap shadows the Homebrew `qemu` binary in `PATH` (`brew link --overwrite qemu-virgl`), and macOS-virgl is not yet in upstream QEMU, so a virgl build can never be the absolute latest QEMU. |
...
for vCPUs — 8 on an 18-core Apple Silicon MacBook Pro, 2 on a 4-core Apple Silicon Mac — plus **8 GB RAM /
...
kernel soft-locks and cloud-init stalls (see the known battle scars in [AGENTS.md](./AGENTS.md)). For the fastest, most reliable fleet
...
To build all distros at once, see
...
The default is the bare distro name — QEMU is the only supported hypervisor.
...
| Firmware | UEFI | UEFI |
| Typical first-boot time (Phase 2, measured) | ~20-48 min | ~0.7-3.5 h (binhost + source builds) |
...
Cloud-init is already running it, and a concurrent `apt`/`emerge`
...
~48 min on Ubuntu and up to ~3.5 h on Gentoo on an idle host; slower
networks and host load may add more). SSH is enabled —
...
- **QEMU (virtio-gpu)**: If the host's QEMU has Venus (Vulkan-over-virtio) compiled in and a working host Vulkan ICD (MoltenVK on macOS), the guest gets hardware-accelerated Vulkan. The launcher passes the ICD via `VK_ICD_FILENAMES` automatically. Otherwise, you get Lavapipe (CPU-side Vulkan via LLVM JIT).
...
`-display cocoa,gl=es` + a GL device (`virtio-gpu-gl-pci`)
on a virgl QEMU build, or plain `-display cocoa` + `virtio-gpu-pci` otherwise. If you need headless operation (e.g. for CI /
fleet runs), edit the launcher to swap:
- `-display cocoa,gl=es` (or `-display cocoa`) → `-display none`
- the GL device (if present) → its non-GL sibling (`virtio-gpu-gl-pci` →
  `virtio-gpu-pci`); the GL-capable
  device needs a GL display backend, so it must be dropped with
  `-display none`
...
guest still runs the `spice-vdagent` channel.
...
python setup_vm.py --ssh-port 2222 --target-dir ~/VMs/q1 &
python setup_vm.py --ssh-port 2224 --target-dir ~/VMs/q2 &
python setup_vm.py --ssh-port 2226 --target-dir ~/VMs/q3 &
```
...
```bash
python setup_vm.py monitor ~/VMs/ubuntu-lts        # live phase bar, 5-sec refresh
...
2. **Per-VM SSH key** — `<target-dir>/ssh_key` (ed25519, generated at
   ...
   ```bash
   ssh -i ~/VMs/ubuntu-lts/ssh_key -p 2222 ubuntu@127.0.0.1
   ```
...
```bash
rm -rf ~/VMs/ubuntu-lts     # QEMU
```

## Re-running

- The cloud image is reused if the URL hasn't changed (Ubuntu daily builds and
  Gentoo rolling releases
  all produce new filenames and trigger a re-download).
- The QEMU disk (qcow2) is regenerated each run.
