...
def _urlopen(url, *, timeout: float = 20, method: Optional[str] = None):
    ...
    flap) back off 4s/8s so a resolver flap that outlives a
...
def _resolve_gentoo(guest_arch: str) -> ResolvedImage:
    ...
    # Retry on transient network flaps is handled inside _urlopen
    # (3 attempts with exponential backoff); just give it a generous
    # per-attempt timeout for the slow Gentoo autobuilds HTML index.
    with _urlopen(listing_url, timeout=60) as r:
...
class ChecksumMismatchError(RuntimeError):
    """The downloaded image's hash does not match the expected digest.

    A subclass of RuntimeError so existing `except RuntimeError` callers
    still catch it, but callers that need to distinguish a stale-cache
    mismatch from a fatal download error can catch this specifically.
    """
...
def verify_hash(
    ...
    if expected is None and sums_url:
        ...
        def _is_our_entry(token: str) -> bool:
            # Path-boundary safe match: "not-<name>" and "<name>.bak" must
            # NOT match (a similarly-named entry would bind the wrong
            # digest to this image); "./dir/<name>", "dir/<name>" and
            # "*<name>" (sha256sum binary mode) must.
            name = token.lstrip("*")
            return name == image.name or name.endswith("/" + image.name)
        ...
        for line in sums_text.splitlines():
            ...
            if not parts:
                continue
            if any(_is_our_entry(p) for p in parts):
                for token in parts:
...
# --------------------------------------------------------------------------
# A single curated GNOME Shell extension (dash-to-dock) is installed into the
# guest from a tagged GitHub-release ZIP by _gs_extensions_common.j2.
# Historically that block curled api.github.com DURING cloud-init, which is
# fragile: the guest's network to GitHub is flaky at first boot (the same
# DNS-flap class of failure that hits distro mirrors), so installs silently
# failed and left extensions "enabled" (via dconf) but not on disk. To make
# installs deterministic we pre-fetch the zip on the HOST (reliable network +
# CA bundle) and serve it to the guest over QEMU's user-net gateway
...
#   * direct_source_url set -> download that URL directly. Served from host cache.
...
def _pick_free_port(start: int, end: int) -> int:
    """Find a free loopback port in [start, end]; raise if none is free.

    Binds 127.0.0.1 (not 0.0.0.0) because the extension server binds
    127.0.0.1: probing all interfaces could return a port that is taken on
    loopback, and the detached http.server would then die silently on its
    own bind failure.
    """
    for p in range(start, end + 1):
        ...
        try:
            ...
            s.close()
    raise RuntimeError(
        f"No free port in range {start}-{end} for the GNOME extension server"
    )
