...
def list_running_qemu_pids(scope: str) -> list[int]:
    ...
    Only swallows pgrep's "no match" (returncode 1) -- any other failure
    (pgrep missing, timeout, broken pipe) propagates so callers don't get
    a silent "VM is dead" when pgrep itself is broken.
    """
    try:
        r = subprocess.run(
    ...
    # pylint: disable-next=try-except-raise
        ...
        # pgrep not on PATH: refuse to claim "no VMs running" -- the
        # caller's preflight / shutdown_verification would skip the
        # kill step and the build would race an existing VM.
        raise
