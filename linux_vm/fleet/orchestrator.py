...
class PhaseAbort(Exception):
    """Raised to short-circuit the current per-VM phase to the cleanup `finally`.

    Used by both build_and_provision and simulate_distro so the
    `shutdown_and_verify` cleanup runs unconditionally; the
    individual functions only differ in what they return afterwards.
    """


def _discover_ssh_port(target: Path, initial_delay: float) -> tuple[int | None, int]:
    """Wait for qemu-system to spawn, then parse its SSH host-forward port.

    Returns (port, waited_sec). `port` is None on timeout; `waited_sec` is
    the total wall time spent so the caller can report it accurately.

    Shared by build_and_provision and simulate_distro (previously ~10
    duplicated lines each).
    """
    time.sleep(initial_delay)
    waited = int(initial_delay)
    for _ in range(30):
...
def build_and_provision(distro: str, gnome_ext_url: str = "") -> tuple[bool, bool]:
    ...
    """
    tag = distro
    target = OUT_ROOT / distro
    build_log = OUT_ROOT / f"{distro}.build.log"
    wait_log = OUT_ROOT / f"{distro}.wait.log"
    stop_log = OUT_ROOT / f"{distro}.stop.log"
    ...
    try:
        ...
        port, waited = _discover_ssh_port(target, 30.0)
        if not port:
            log_master(f"{tag}: phase-2 abort: couldn't find qemu hostfwd port after {waited}s")
            raise PhaseAbort
        host = "127.0.0.1"
...
def simulate_distro(distro: str) -> dict:
    ...
    """
    target = OUT_ROOT / "simulate" / distro
    target.mkdir(parents=True, exist_ok=True)
    sim_log_dir = OUT_ROOT / "simulate" / "logs"
    sim_log_dir.mkdir(parents=True, exist_ok=True)
    build_log = sim_log_dir / f"{distro}-simulate-build.log"
    wait_log = sim_log_dir / f"{distro}-simulate-wait.log"
    stop_log = sim_log_dir / f"{distro}-simulate-stop.log"
    username = USERNAMES[distro]
    ssh_key = target / "ssh_key"
    host: str = ""
    port: int = 0
    ...
    log_master(f"--- simulate {distro}: building simulate-mode VM")
    ...
    try:
        ...
        # Phase 2a: discover SSH endpoint
        port, waited = _discover_ssh_port(target, 90.0)
        if not port:
            raise PhaseAbort(f"couldn't find qemu hostfwd port in {waited}s")
        host = "127.0.0.1"
        log_master(f"--- simulate {distro}: SSH endpoint = {username}@{host}:{port}")
        ...
        if reachable_host is None:
            raise PhaseAbort(f"SSH never reachable on {host}:{port} in {SSH_REACHABLE_TIMEOUT_SEC//60} min")
        host = reachable_host
        mirror = DISTRO_MIRROR.get(distro, "")
        if mirror:
            dns_ok = check_guest_dns(host, port, username, str(ssh_key), mirror)
            if not dns_ok:
                log_master(f"  {distro}: WARN: guest DNS resolution failed for {mirror}")
        ...
        if rc2 == -1:
            raise PhaseAbort(f"cloud-init wait TIMEOUT after {el2/60:.1f} min")
        ...
        if vm_started:
            try:
                ...
                log_master(f"--- simulate {distro}: shutdown error: {type(e).__name__}: {e}")
...
def simulate_all(distros: list[str]) -> dict:
    ...
    for distro in distros:
        idx += 1
        log_master(f"--- simulate {idx}/{len(distros)}: {distro} ---")
        r = simulate_distro(distro)
        results[distro] = r
        log_master(f"--- simulate {distro}: {r['status']} -- {r['detail']}")
        if r["status"] != "PASS":
            log_master(f"!!! simulate FAIL: {distro} -- inspect {OUT_ROOT / 'simulate' / 'logs' / f'{distro}-simulate-wait.log'}")
    return results
