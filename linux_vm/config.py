...
_BASE_DEFAULTS: dict[str, object] = {
    ...
    # give it headroom without slowing the apt distros.
...
# Per-distro "what cloud-init will now" summary printed at the end of
# `setup_vm.py`. Each value is a bullet list shown after the build.
# `ga` (guest agent) is a token substituted into the summary at
# print time so a single change updates both distros.
DISTRO_INSTALL_SUMMARY: dict[str, str] = {
    "ubuntu-lts": (
        "  1. apt update + apt upgrade the Ubuntu LTS base system\n"
        "  2. Install Ubuntu LTS desktop (ubuntu-desktop-minimal), "
        "GNOME display manager, {ga}, rclone\n"
    ...
    "gentoo": (
        ...
        "  2. emerge --getbinpkg gnome-base/gnome + individual GNOME apps + {ga}\n"
...
# Per-VM artifact filenames inside the target directory. Used by the
# orchestrator (writes the qcow2 + launcher) and by the monitor / fleet
# builder (which look for them by name to read state back out).
