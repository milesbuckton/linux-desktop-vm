...
def _build_qemu_argv(
    ...
    # Probe the SAME binary that will be executed. _ensure_qemu_app copies
    # the raw binary into a signed bundle; the raw source binary may itself
    # be un-executable (e.g. taskgated SIGKILLs a broken adhoc signature),
    # and probing it made every capability check fail, silently downgrading
    # the launcher to the non-GL virtio-gpu-pci (llvmpipe, no Venus).
    bundle_qemu = _ensure_qemu_app(qemu)
    argv: list[str] = [str(bundle_qemu)]
    ...
    # flags).
    gfx = _argv_graphics(bundle_qemu, guest_arch)
    argv += gfx
    ...
    argv += _argv_spice(bundle_qemu, guest_arch)
