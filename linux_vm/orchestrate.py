...
from .config import (
    VMConfig, DISTRO_DEFAULTS, DISTRO_INSTALL_SUMMARY, banner,
    filename_from_url, DISK_FILENAME, LAUNCHER_FILENAME,
)
...
def _extract_archive_safely(qcow2: Path, target_dir: Path) -> None:
    """Extract an archived cloud image, rejecting unsafe members.
    ...
    (tar-slip / path-traversal CVE class).

    Python's tarfile is the SINGLE validation source for both extraction
    paths: it sees member TYPES, which the plain `tar -t` name listing
    cannot -- a symlink member followed by a write-through member escapes
    target_dir even when every member NAME is benign. Symlink and
    hardlink members are therefore rejected outright, on every Python
    version (the 3.12+ `filter="data"` extractall path is a second layer,
    not the only one).
    """
    ...
    try:
        ...
        raise RuntimeError(
            f"Could not read archive {qcow2} for safety validation: {exc}"
        ) from exc

    unsafe = [
        m.name for m in members
        if not _member_is_safe(m.name) or m.issym() or m.islnk()
    ]
    if unsafe:
        raise RuntimeError(
            "Archive contains unsafe member(s) (path traversal or link), "
            f"refusing to extract: {unsafe[:3]}"
        )

    try:
        # Fast path: the system tar's extraction. Member safety was already
        # validated above; macOS bsdtar additionally refuses to extract
        # through symlinks on its own.
...
def parse_args(default_target: Path) -> argparse.Namespace:
    ...
    args = p.parse_args()
    # The host-served extension URL is a single source of truth: only the
    # fleet orchestrator's per-VM hostfwd-style loopback URL is supported.
    # Refuse anything else so an arbitrary URL can't be smuggled into the
    # seed ISO via the Jinja context. sys.exit here (not return) because
    # main() uses the returned Namespace unconditionally.
    if args.gnome_ext_url is not None and not args.gnome_ext_url.startswith("http://10.0.2.2:"):
        log(f"--gnome-ext-url must be http://10.0.2.2:<port>; got {args.gnome_ext_url!r}", "err")
        sys.exit(2)
    return args
...
def main() -> int:
    ...
    if not is_supported_host_arch(host.arch):
        log("This script supports arm64 (Apple Silicon) hosts only.", "err")
        return 2
...
def _build_one_vm(args, host) -> int:
    ...
    # ---- Prerequisites: pycdlib + certifi + jinja2 ---------------------
    # These MUST run before the first network call (distro.resolve below):
    # certifi is the CA bundle used by _urlopen, and the resolve step
    # hits distro mirrors (e.g. changelogs.ubuntu.com) whose cert is in
    # the certifi bundle but not the system store. If ensure_certifi
    # runs AFTER the first _urlopen, that first call fails outright on
    # SSL verification -- no retry, just a hard error.
    ...
    disk_path = cfg.target_dir / DISK_FILENAME
    def_path = cfg.target_dir / LAUNCHER_FILENAME

    # ---- Extract archived images (.tar.xz containing disk.raw) ----------
    # Path.parents[-1].suffix is ".tar.xz" for a tarball, ".xz" for a
    # bare-xz compressed image. suffix only returns the LAST suffix
    # segment, so we also check the file name to cover .tar.xz.
    if qcow2.suffix.lower() == ".xz" or qcow2.name.lower().endswith(".tar.xz"):
        ...
        # A reused target dir can hold a stale raw/qcow2 from a previous
        # build, and "largest file wins" alone would happily pick it. The
        # archive's own mtime is the boundary -- extraction always lands
        # after the download completes. Fall back to the unfiltered list if
        # nothing qualifies (clock skew, copied-in files).
        arch_mtime = qcow2.stat().st_mtime
        fresh = [c for c in candidates if c.stat().st_mtime >= arch_mtime]
        if fresh:
            candidates = fresh
    ...
    if ssh_pub_path.exists():
        ...
        # ssh-keygen -N "" creates the private key 0o600 by default, but be
        # explicit (and tolerate a host that mints a looser mode for any
        # reason): the private key is what lets anyone log in as the user.
        try:
    ...
    # Contains the cleartext user + root password, so it must NOT be
    # world-readable, even for a microsecond. write_text() creates the
    # file with mode 0o644 then chmods; an OS-level file watcher could
    # read the contents in that window. Open with O_CREAT|O_WRONLY and an
    # explicit 0o600 mode instead, then write via the fd.
    ...
    try:
        info_fd = os.open(str(info_path), os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600)
    except OSError as e:
        log(f"Could not open {info_path} for writing: {e}", "err")
        return 7
    try:
        with os.fdopen(info_fd, "w", encoding="utf-8") as fh:
            fh.write(info_text)
    except OSError as e:
        log(f"Could not write {info_path}: {e}", "err")
        return 7
    ...
    # Passwords go to install-info.txt (0600). Only echo them on a real
    # terminal: the fleet orchestrator captures this stdout into
    # <distro>.build.log, and passwords must not land in world-readable
    # build logs.
    if sys.stdout.isatty():
        print(f"{C.BOLD}Password:  {C.RESET} {cfg.password}")
        print(f"{C.BOLD}Root pw:   {C.RESET} {cfg.root_password}")
    else:
        print(f"{C.BOLD}Passwords: {C.RESET} (not printed to non-TTY output -- see {info_path})")
    print(f"{C.BOLD}Definition:{C.RESET} {def_path}")
    print(f"{C.BOLD}Info:      {C.RESET} {info_path}")
    # Guest agent installed by every template's vendor-app block. Single
    # token substituted into the per-distro summary string from config.
    ga = "spice-vdagent"
    steps = DISTRO_INSTALL_SUMMARY.get(args.distro, "").format(ga=ga)
    print(
        textwrap.dedent(
            f"""
            cloud-init will now:
