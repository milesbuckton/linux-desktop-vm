...
- **Firefox is deliberately not installed**: `ubuntu-desktop-minimal` doesn't pull it, and no template adds it. Note the old `snapd` pin at −1 in `99-no-bloat.pref` is GONE — `snapd`/`snap-store`/`bingwall` are now deliberately installed (apt `snapd` + post_runcmd snap installs), so don't "restore" that pin: it would break the snap installs. Epiphany is the browser everywhere.
