...
# Capture raw output first (includes stderr) so we can detect errors that
# occur before any device lines are emitted.
RAW="$("${SSH[@]}" "$SSH_USER@127.0.0.1" \
  'vulkaninfo --summary 2>&1' 2>/dev/null)"
INST=$(echo "$RAW" | grep -iE "deviceName|apiVersion|driverName" | head -20)
echo "$INST"

if ! echo "$INST" | grep -iqE "venus|virtio"; then
  if echo "$RAW" | grep -qiE "error|fail"; then
    echo "VENUS-FAIL: vulkaninfo reported an error (likely VK_ERROR_OUT_OF_HOST_MEMORY / ring):" >&2
    echo "$RAW" | grep -iE "error|fail" >&2
    exit 1
  fi
