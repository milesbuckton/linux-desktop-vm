...
from linux_vm.audit import gentoo_atoms_from_text
...
def extract_packages(distro: str) -> set[str]:
    ...
    for match in install_cmd_re.finditer(rc_text):
        ...
        for tok in tokens:
            ...
            # from the [sh, -c, "apt-get install ..."] format.
            tok = tok.strip("]\"'")
    ...
    # Gentoo: emerge calls in runcmd + gentoo-install.sh (write_file) use
    # the same multi-line atom grammar that lint-templates.py parses, so
    # both scripts see the same set -- keep the audit honest with the
    # build's actual install list.
    if distro == "gentoo":
        pkgs.update(gentoo_atoms_from_text(rc_text))
        for wf in loaded.get("write_files") or []:
            if isinstance(wf, dict) and wf.get("path", "").endswith("gentoo-install.sh"):
                pkgs.update(gentoo_atoms_from_text(wf.get("content") or ""))
